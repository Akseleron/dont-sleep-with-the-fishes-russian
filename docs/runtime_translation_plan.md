...
## Runtime Texture And Fishing Follow-Up

Safety point:

- Created branch `backup/before-codex-runtime-textures`.
- Created tag `before-codex-runtime-textures`.

Runtime-visible text fixes:

- Added manual runtime key:
  - `Laurel does not feel well...` -> `Лорел плохо...`
- Fixed dialogue QA issues:
  - `C- could you lend it to me?` -> `Н- но ты сможешь отдать её мне?`
  - `You saved my life, thank you.. I- I will repay it after we.. you know.. survive. B- But I am sure we will.` -> `Ты спас мне жизнь, спасибо... Я-я когда-нибудь отплачу тебе.`
- Updated `translations/resources_textasset_translations.tsv` for the same two dialogue fixes so the resource translation cache no longer preserves the bad drafts.
...
- The regex file is intentionally outside `_AutoGeneratedTranslations.txt` because XUnity ignores regexes in generated `_` files.
- Current regex experiment:
  - `Weight: Xkg` -> `Вес: X кг`
...
- Added and committed `scripts/export_unity_textures.py` separately as `add Unity texture export tooling`.
- Added `scripts/select_texture_candidates.py`.
- Exported from `game_runtime_test/DontSleepWithTheFishes_Data` into `debug_reports/texture_export`.
- Export result:
  - `asset_files=8`
  - `rows=878`
  - `exported=869`
  - `failed=9`
- Texture names are mostly `unnamed_*`, so keyword selection found no narrow candidates.
- Selector fallback copied all exported PNGs into `debug_reports/texture_candidates_for_redraw` and wrote `candidate_index.tsv` with `fallback_all_unnamed_export`.
- If the candidate folder is too broad for redraw work, use `debug_reports/texture_export/images` as the complete export and visually sort from there.

Instructions for redrawn PNGs:
...
- Do not crop edges.
- Save as PNG.
- Put edited files in a separate `redrawn/` folder.
